...
- `skills/setup-autoresearch/SKILL.md`: a skill that inspects a target repo, asks only high-risk follow-up questions, and generates a repo-specific `program.md`
...
5. stop and ask whether you want to start immediately or use the shell runner

The generated `program.md` should make the keep/discard rule explicit:

- if the measure improves, commit the change to git
- if the measure stays the same with simpler logic, commit the change to git
- otherwise discard the change from git
