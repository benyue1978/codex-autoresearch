...
NOW_EPOCH=$(date -d '2026-03-27 00:25:07' +%s)
...
if [[ "$call_count" == "1" ]]; then
  : > "$output_file"
  printf '%s\n' '{"type":"error","message":"You'\''ve hit your usage limit. Upgrade to Pro (https://chatgpt.com/explore/pro), visit https://chatgpt.com/codex/settings/usage to purchase more credits or try again at 12:28 AM."}'
  exit 1
fi
...
grep -q '^173$' "$SLEEP_LOG"
grep -q 'detected Codex usage limit; sleeping 173s until 12:28 AM before retry' "$STDERR_LOG"
